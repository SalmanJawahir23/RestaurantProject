...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(0, 10, "Central Ceylon Restaurant Report", 0, 1, 'C');
        $this->Ln(6); // spacing
    }
    ...
    function ChapterBody($body)
    {
        ...
        $this->MultiCell(0, 6, $body); // line height
        $this->Ln(2); // spacing
    }
...
{
    $columnWidths = array(30, 40, 50, 70); // column widths
...
$pdf->ChapterBody("Total Revenue Today: LKR " . number_format($totalRevenueToday, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Week: LKR " . number_format($totalRevenueThisWeek, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Month: LKR " . number_format($totalRevenueThisMonth, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Year: LKR " . number_format($totalRevenueThisYear, 2));
$pdf->Ln();
...
$header = array('Date','Day' , 'Revenue (LKR)');
$data = array();
...
$header = array('Date','Week' , 'Revenue (LKR)');
$data = array();
...
$header = array('Date','Month' , 'Revenue (LKR)');
$data = array();
...
$header = array('Date','Day' , 'Item Category', 'Revenue (LKR)');
$data = array();
while ($row = mysqli_fetch_assoc($dailyCategoryRevenue)) {
...
$header = array('Date','Week' , 'Item Category', 'Revenue (LKR)');
$data = array();
while ($row = mysqli_fetch_assoc($weeklyCategoryRevenue)) {
...
$header = array('Date','Month' , 'Item Category', 'Revenue (LKR)');
$data = array();
while ($row = mysqli_fetch_assoc($monthlyCategoryRevenue)) {
...
$header = array('Date', 'Item Category', 'Revenue (LKR)');
$data = array();
