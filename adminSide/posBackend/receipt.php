...
// Header
$pdf->Cell(0, 10, "Central Ceylon Restaurant", 0, 1, 'C');
...
while ($item_row = mysqli_fetch_assoc($items_result)) {
    ...
    $pdf->Cell(40, 10, 'LKR ' . number_format($item_price,2), 1);
    $pdf->Cell(40, 10, $quantity, 1);
    $pdf->Cell(40, 10, 'LKR ' . number_format($total,2), 1);
    $pdf->Ln();
}
...
$payment_amount = !empty($_GET['payment_amount']) ? $_GET['payment_amount'] : $after_tax;
$balance = $payment_amount - $after_tax;
...
$pdf->Cell(40, 10, 'LKR ' . number_format($before_tax,2), 1);
$pdf->Ln();

$pdf->Cell(150, 10, 'Tax (' . ($tax_rate * 100) . '%)', 1);
$pdf->Cell(40, 10, 'LKR ' . number_format($tax_amount,2), 1);
$pdf->Ln();

$pdf->Cell(150, 10, 'Grand Total', 1);
$pdf->Cell(40, 10, 'LKR ' . number_format($after_tax,2), 1);
$pdf->Ln();

$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(150, 10, 'Cash', 1);
$pdf->Cell(40, 10, 'LKR ' . number_format($payment_amount,2), 1);
$pdf->Ln();

$pdf->Cell(150, 10, 'Balance', 1);
$pdf->Cell(40, 10, 'LKR ' . number_format($balance,2), 1);
$pdf->Ln();
